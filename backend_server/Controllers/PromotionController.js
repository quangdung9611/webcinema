    ...
    try {
        ...
        if (page !== undefined || limit !== undefined) {
            return res.status(400).json({
                success: false,
                message: 'Route /api/promotions không hỗ trợ page/limit. Vui lòng dùng /api/promotions/paginated'
            });
        }
...
exports.createPromotion = async (req, res) => {
    try {
        const promotionId = await PromotionService.createPromotion(req.body, req.files || {});
...
exports.updatePromotion = async (req, res) => {
    try {
        const { promotion_id } = req.params;
        await PromotionService.updatePromotion(promotion_id, req.body, req.files || {});
        return res.status(200).json({
            success: true,
...
=========================================================*/
exports.likePromotion = async (req, res) => {
    try {
        const { promotion_id } = req.params;
        ...
        console.error('Like Promotion Error:', err);
        return res.status(err.statusCode || 500).json({
            success: false,
            message: err.message || 'Lỗi máy chủ'
        });

...
const AuthService = require("../Services/AuthService"); // Import để dùng loginAfterRegistration
...
exports.getAllUsers = async (req, res) => {
    try {
        const data = await UserService.getAllUsersAll(req.query.search || "");
...
exports.createUser = async (req, res) => {
    try {
        ...
        return res.status(err.statusCode || 500).json({ success: false, field: err.field || null, message: err.message || "Lỗi máy chủ" });
    }
};
...
exports.updateUserProfile = async (req, res) => {
    try {
        ...
        return res.status(err.statusCode || 400).json({ success: false, field: err.field || null, message: err.message || "Lỗi máy chủ" });
    }
};
...
// Thiết lập PIN lần đầu (Sau khi lưu PIN -> Tự động đăng nhập)
exports.setupPin = async (req, res) => {
    try {
        ...
        // 2. Hoàn tất đăng ký: Lấy user và tự động đăng nhập
        const user = await UserService.completeRegistration(req.user.user_id);
        const authResult = await AuthService.loginAfterRegistration(user, req, res);

        return res.status(200).json({
            success: true,
            message: "Đăng ký và thiết lập mã PIN thành công!",
...
exports.verifyPin = async (req, res) => {
    try {
        ...
        return res.status(err.statusCode || 400).json({ success: false, code: err.code || null, message: err.message || "Lỗi máy chủ" });
    }
};
...
exports.updatePin = async (req, res) => {
    try {
        ...
        if (!oldPin || !newPin) {
            return res.status(400).json({ success: false, field: !oldPin ? "oldPin" : "newPin", message: "Vui lòng nhập đầy đủ PIN cũ và PIN mới" });
        }

        await UserService.updatePin(req.user.user_id, oldPin, newPin);
        return res.status(200).json({ success: true, message: "Đổi mã PIN thành công!" });
    } catch (err) {

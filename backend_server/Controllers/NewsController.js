...
const deleteFile = (fileName) => {
    ...
    // Đường dẫn vào thư mục uploads/news/
...
getAllNews: async (req, res) => {
    try {
        const query = `
            SELECT 
                news_id, title, slug, image_url, views, created_at,
                -- Vẫn giữ short_content của SQL làm phương án dự phòng
                IF(LENGTH(content) > 200, CONCAT(LEFT(content, 200), '...'), content) AS short_content,
                -- Lấy thêm content gốc để Frontend xử lý sạch thẻ HTML
                content
            FROM news 
    ...
    // 2. Lấy chi tiết bài viết theo Slug (User)
    ...
    // 4. Thêm bài viết mới (Dùng Transaction giống MovieController)
    createNews: async (req, res) => {
        ...
        try {
            ...
            const sql = 'INSERT INTO news (title, slug, content, image_url) VALUES (?, ?, ?, ?)';
            ...
            if (req.file) deleteFile(req.file.originalname); // Xóa ảnh nếu insert lỗi
    ...
    // 5. Cập nhật bài viết (Dùng Transaction + Xóa ảnh cũ giống MovieController)
    updateNews: async (req, res) => {
        ...
        try {
            ...
            // 1. Kiểm tra bài viết tồn tại và lấy ảnh cũ
            ...
            if (req.file) {
                deleteFile(old[0].image_url); // Xóa ảnh cũ trên server
                finalImage = req.file.originalname; // Dùng ảnh mới
            } else if (image_url) {
                finalImage = image_url; // Dùng lại ảnh cũ gửi từ client (nếu có)
            }
    ...
    deleteNews: async (req, res) => {
        ...
        try {
            ...
            if (news.length > 0) {
                deleteFile(news[0].image_url); // Xóa ảnh khi xóa bài viết
            }

...
const validateGenre = (data) => {
    ...
    if (!genre_name || genre_name.trim() === "") return "Vui lòng nhập tên thể loại.";
    if (genre_name.trim().length < 2) return "Tên thể loại phải từ 2 ký tự trở lên.";
    if (genre_name.trim().length > 50) return "Tên thể loại không được vượt quá 50 ký tự.";
    return null;
};
...
class GenreService {
    ...
    async createGenre(data) {
        ...
        const duplicate = await GenreRepository.findByNameOrSlug(data.genre_name.trim(), slug);
        if (duplicate) {
    ...
    async updateGenre(genreId, data) {
        ...
        const slug = createSlug(data.genre_name);
        const duplicate = await GenreRepository.findByNameOrSlug(data.genre_name.trim(), slug, genreId);
        if (duplicate) {
            const err = new Error("Tên thể loại đã tồn tại.");
            err.statusCode = 400;
            throw err;

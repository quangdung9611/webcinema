...
const validateBlogData = (data, file, isUpdate = false) => {
    ...
    if (description.trim().length < 10) return "Mô tả quá ngắn.";
    if (likes !== undefined && likes !== "") {
        const num = parseInt(likes);
        if (isNaN(num) || num < 0) return "Likes không hợp lệ.";
    }
    if (!isUpdate && !file) return "Vui lòng upload ảnh.";
    return null;
};
...
class BlogCinemaService {
    ...
    async updateBlog(blogId, data, file) {
        ...
        const slug = createSlug(title);
        const exists = await BlogCinemaRepository.existsByTitleOrSlug(title.trim(), slug, blogId);
        if (exists) {
    ...
    async deleteBlog(blogId) {
        ...
        try {
            ...
            await BlogCinemaRepository.deleteWithConnection(conn, blogId);
            await BlogCinemaRepository.commit(conn);
            return true;
        } catch (err) {
            await BlogCinemaRepository.rollback(conn);

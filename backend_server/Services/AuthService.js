...
exports.login = async (email, password, rememberMe = false, req, res) => {
    ...
    if (lockInfo && lockInfo.isLocked) {
        throw {
            ...
            message: `Tài khoản đã bị khóa ${lockInfo.lockDurationText}. Vui lòng thử lại sau.`,
            data: { level: lockInfo.level, remainingSeconds: lockInfo.remainingSeconds, lockDuration: lockInfo.lockDuration, lockDurationText: lockInfo.lockDurationText, maxAttempts: lockInfo.maxAttempts, lockedUntil: lockInfo.lockedUntil }
        };
    }
    ...
    if (!user) {
        ...
        if (attempts >= 5) {
            ...
            throw { statusCode: 429, code: 'ACCOUNT_LOCKED', message: `Bạn đã nhập sai quá 5 lần. Tài khoản đã bị khóa ${text}.`, data: { level: newLevel, remainingSeconds: duration, lockDuration: duration, lockDurationText: text, maxAttempts: 5, lockedUntil: Date.now() + duration * 1000 } };
        }
    ...
    if (ioInstance && user.user_id) {
        ioInstance.to(`user_${user.user_id}`).emit('session_expired', {
            ...
            newDevice: { ip: req.ip || req.connection?.remoteAddress || 'Unknown', userAgent: req.headers?.['user-agent']?.substring(0, 100) || 'Unknown' },
    ...
    await RefreshTokenRepository.create({
        user_id: user.user_id, token_hash: accessTokenHash,
    ...
    return {
        ...
        user: { user_id: user.user_id, username: user.username, full_name: user.full_name, email: user.email, phone: user.phone, role: user.role, points: user.points, email_verified: user.email_verified }
    };
};

/*=========================================================
    🆕 REGISTER STEP 1 (ĐĂNG KÝ TÀI KHOẢN)
=========================================================*/
...
exports.logout = async (req, res) => {
    ...
    } catch (error) { console.error('❌ [LOGOUT] Lỗi khi xóa socket:', error.message); }
...
exports.forgotPassword = async (email, req) => {
    ...
    const user = await UserRepository.findByEmail(email);
    if (!user) return { success: true, message: "Nếu email này tồn tại, chúng tôi đã gửi liên kết đặt lại mật khẩu." };
    ...
    return { success: true, message: "Nếu email này tồn tại, chúng tôi đã gửi liên kết đặt lại mật khẩu." };
};
...
exports.submitNewPassword = async (token, newPassword) => {
    ...
    return { success: true, message: "Mã OTP xác nhận đã được gửi tới email của bạn.", email: user.email };
};
...
exports.verifyEmail = async (verifyToken) => {
    ...
    return {
        ...
        user: { user_id: user.user_id, email: user.email, full_name: user.full_name, email_verified: 1 }
    };
};
...
exports.getActiveDevices = async (userId) => {
    ...
    return {
        ...
        devices: tokens.map(token => ({
            device_id: token.token_id, device_name: token.device_name || "Unknown Device", ip_address: token.ip_address || "Unknown", last_used_at: token.last_used_at || token.created_at, created_at: token.created_at, expires_at: token.expires_at, is_current: false
        }))
    };
};

...
class ActorService {
    ...
    async createActor(data, file) {
        ...
        const slug =
            providedSlug && providedSlug.trim()
                ? createSlug(providedSlug)
                : createSlug(name);

        // CHECK DUPLICATE
        const exists = await ActorRepository.existsByNameOrSlug(
            name.trim(),
            slug
        );
    ...
    async updateActor(actorId, data, file) {
        // GET EXISTING ACTOR
        const existing = await ActorRepository.findById(
            actorId
        );

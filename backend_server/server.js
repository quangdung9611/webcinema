...
// ✅ FIX: Chọn token theo origin — admin ưu tiên admin_token
// ============================================================
...
io.use(async (socket, next) => {
    try {
        ...
        // ========================================================
        // ✅ FIX: GET ACCESS TOKEN — CHỌN THEO ORIGIN
        // ========================================================
        ...
        let token;
        if (isAdminOrigin) {
            // Admin domain → ưu tiên admin_token
            ...
            // User domain → ưu tiên user_token
        ...
        try {
            ...
            const validToken = await RefreshTokenRepository.findValidTokenHash(
                accessTokenHash
            );
...
io.on("connection", async (socket) => {
    ...
    socket.on("client-chon-ghe", async (data) => {
        try {
            ...
            if (!showtimeId || !seatId) {
                console.warn(`⚠️ [SOCKET] Invalid seat data from ${socketId}`);
    ...
    socket.on("request-holding-seats", async (data) => {
        try {
            const showtimeId = Number(
                data?.showtimeId || socket.handshake.query?.showtimeId
            );
    ...
    socket.on("clear_all_holding_seats", async (data) => {
        try {
            ...
            if (data?.showtimeId) {
                clearedCount =
                    await CacheService.releaseShowtimeSeatLocksByOwner(
                        Number(data.showtimeId),
                        ownerToken
                    );
            } else {
                clearedCount = await CacheService.releaseAllSeatLocksByOwner(
                    ownerToken
                );
            }
    ...
    socket.on("disconnect", async () => {
        ...
        try {
            const releasedCount =
                await CacheService.releaseAllSeatLocksByOwner(ownerToken);
        ...
        if (userId) {
            try {
                const registeredSocket = await CacheService.getUserSocket(
                    userId
                );

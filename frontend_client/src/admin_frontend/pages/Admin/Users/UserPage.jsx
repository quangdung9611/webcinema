...
const UserPage = () => {
    ...
    // ======================================================
    // CHỐNG GỌI TRÙNG (chỉ dùng isFetching + AbortController)
    // ======================================================
    ...
    // ======================================================
    // FETCH USERS - ĐƠN GIẢN, CHỐNG GỌI ĐỒNG THỜI
    // ======================================================
    ...
    // ======================================================
    // SEARCH - DEBOUNCE 400ms, CHỈ GỌI KHI THỰC SỰ THAY ĐỔI
    // ======================================================
    ...
    const handleChange = (e) => {
        ...
        if (name === 'user_avatar') {
            setUserAvatarFile(files?.[0] || null);
            return;
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            Object.entries(formData).forEach(([key, value]) => {
                if (key === 'password' && editingUser && !value) return;
        ...
        showAlert(
            ...
            async () => {
                try {
                    ...
                    const newPage = users.length === 1 && currentPage > 1
                        ? currentPage - 1
                        : currentPage;
                    ...
                    showAlert('Lỗi', error.response?.data?.error || 'Không thể xóa người dùng.', 'error');
                }
            },
            closeAlert
        );
    ...
    // ======================================================
    const columns = [
        { title: 'ID', key: 'user_id', render: (row) => `#${row.user_id}` },
        {
            title: 'Avatar',
            key: 'user_avatar',
            render: (row) => {
                if (!row.user_avatar) return <span style={{ color: '#888' }}>—</span>;
                const avatarSrc = getAvatarUrl(row.user_avatar);
                return (
                    <img
                        src={avatarSrc}
                        alt="avatar"
        ...
        { title: 'Điểm', key: 'points' },
        {
            title: 'Vai trò',
            key: 'role',
            render: (row) => <span className={`status-badge ${row.role}`}>{row.role}</span>
        },
        {
            title: 'Thao tác',
            key: 'actions',

...
const Booking = () => {
    ...
    useEffect(() => {
            ...
            if (data.locked === false || data.success === false) {
                ...
                setSeats(prev => prev.map(seat =>
                    Number(seat.seat_id) === seatId
                        ? { ...seat, is_locked_by_user: false, held_by_other: true }
                        : seat
                ));
                showErrorModal(
                    'Ghế đã có người chọn',
                    data.message || `Ghế ${seatId} vừa được người khác giữ. Vui lòng chọn ghế khác.`
                );
                return;
            }
        ...
        currentSocket.on('server-seat-lock-error', handleSeatLockError);

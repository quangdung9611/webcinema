...
const Booking = () => {
    ...
    const getCouplePair = useCallback((currentSeat, allSeats) => {
        ...
        const nextSeat = allSeats.find((s) =>
            s.seat_id !== currentSeat.seat_id && isCoupleSeat(s) &&
            s.seat_row === currentSeat.seat_row && Number(s.seat_number) === currentNumber + 1
        );
    ...
    const isCoupleDisplaySeat = useCallback((seat) => {
        ...
        const previousSeat = seats.find((s) =>
            isCoupleSeat(s) && s.seat_row === seat.seat_row && Number(s.seat_number) === currentNumber - 1
        );
        ...
        try {
            ...
            if (savedSeats && savedShowtime === showtimeId.toString()) {
                ...
                seatsData = seatsData.map((s) => {
                    const isSelected = parsed.some((p) => Number(p.seat_id) === Number(s.seat_id));
                    if (isSelected) {
                        return { ...s, is_locked_by_user: true, held_by_other: false };
                    }
                    return s;
                });
    ...
    useEffect(() => {
        ...
        const handleSeatLocked = (data) => {
            ...
            const isOwnSeat = selectedSeats.some((s) => Number(s.seat_id) === Number(data.seatId));
            setSeats((prev) =>
                prev.map((s) =>
                    Number(s.seat_id) === Number(data.seatId)
                        ? { ...s, is_locked_by_user: true, held_by_other: !isOwnSeat }
                        : s
                )
            );
        };
        ...
        const handleSeatUnlocked = (data) => {
            ...
            setSeats((prev) =>
                prev.map((s) =>
                    Number(s.seat_id) === Number(data.seatId)
                        ? { ...s, is_locked_by_user: false, held_by_other: false }
                        : s
                )
            );
        };
        ...
        const handleSeatList = (seatList) => {
            ...
            if (savedSeats) {
                const parsed = JSON.parse(savedSeats);
                ownSeatIds = parsed.map(p => Number(p.seat_id));
            }
            setSeats((prev) => {
                const updated = [...prev];
                seatList.forEach((data) => {
                    if (Number(data.showtimeId) === Number(showtimeId)) {
                        const index = updated.findIndex(s => Number(s.seat_id) === Number(data.seatId));
                        if (index !== -1) {
                            const isOwnSeat = ownSeatIds.includes(Number(data.seatId));
                            updated[index] = {
                                ...updated[index],
    ...
    const clearBookingSession = useCallback(() => {
        selectedSeats.forEach((s) => {
            if (isSocketConnected) {
                socketService.emit('client-huy-chon-ghe', { seatId: s.seat_id, showtimeId });
            }
        });
        ...
        setSelectedSeats([]);
    }, [selectedSeats, isSocketConnected, showtimeId]);

    // ========================================================= HANDLE SEAT CLICK
    const handleSeatClick = useCallback((seat) => {
        if (!seat) return;
        if (seat.seat_status === 'Booked' || Number(seat.is_active) === 0 || seat.is_locked_by_user || seat.held_by_other) {
            return;
        }
        if (!isSocketConnected) {
            setModalConfig({
        ...
        if (couple) {
            ...
            if (pairSeat.seat_status === 'Booked' || Number(pairSeat.is_active) === 0 || pairSeat.is_locked_by_user || pairSeat.held_by_other) {
                setModalConfig({
                    show: true,
                    type: 'error',
        ...
        const allSelected = seatsToToggle.every((targetSeat) =>
            selectedSeats.some((s) => Number(s.seat_id) === Number(targetSeat.seat_id))
        );

        // HỦY CHỌN
        if (allSelected) {
            const updated = selectedSeats.filter(
                (selectedSeat) => !seatsToToggle.some((targetSeat) => Number(targetSeat.seat_id) === Number(selectedSeat.seat_id))
            );
            seatsToToggle.forEach((targetSeat) => {
                if (isSocketConnected) {
                    socketService.emit('client-huy-chon-ghe', { seatId: targetSeat.seat_id, showtimeId });
                }
            });
            ...
            setSeats((prev) =>
                prev.map((s) => {
                    const isInUpdated = updated.some((u) => Number(u.seat_id) === Number(s.seat_id));
                    const wasSelected = selectedSeats.some((old) => Number(old.seat_id) === Number(s.seat_id));
                    if (wasSelected && !isInUpdated) {
                        return { ...s, is_locked_by_user: false, held_by_other: false };
                    }
                    return s;
                })
            );
        ...
        // KIỂM TRA GIỚI HẠN
        const newSeatCount = selectedSeats.length + seatsToToggle.length;
        if (newSeatCount > 8) {
            setModalConfig({
                show: true,
                type: 'error',
        ...
        seatsToToggle.forEach((targetSeat) => {
            const alreadyExists = updated.some((s) => Number(s.seat_id) === Number(targetSeat.seat_id));
            if (!alreadyExists) {
                updated.push(targetSeat);
                if (isSocketConnected) {
                    socketService.emit('client-chon-ghe', { seatId: targetSeat.seat_id, showtimeId });
                }
            }
        });
        ...
        setSelectedSeats(updated);
        sessionStorage.setItem('selectedSeats', JSON.stringify(updated));
    }, [selectedSeats, seats, isSocketConnected, showtimeId, isCoupleSeat, getCouplePair]);

    // ========================================================= CONTINUE
    const handleContinue = () => {
        setIsNavigating(true);
        navigate('/foods', {
            state: { movie, selectedCinema, selectedDate, selectedShowtime, selectedSeats, showtimeDetail }
        });
                                                            ...
                                                            {displaySeats.map((seat) => {
                                                                ...
                                                                return (
                                                                    <Seat
                                                                        key={seat.seat_id}
                                                                        type={seat.seat_type}
                                                                        selected={selectedSeats.some((s) => Number(s.seat_id) === Number(seat.seat_id))}

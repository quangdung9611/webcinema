...
const MomoApp = () => {
    ...
    const getBookingData = () => {
        ...
        // --------------------------------------------------------
        // Ưu tiên state nếu đã có tempBookingId
        // --------------------------------------------------------
        ...
        try {
            ...
            if (tempId) {
                return {
                    ...
                    customerEmail:
                        localStorage.getItem('momoCustomerEmail') || '',

                    customerName:
                        localStorage.getItem('momoCustomerName') || '',

                    customerPhone:
                        localStorage.getItem('momoCustomerPhone') || '',

                    totalAmount:
                        parseFloat(
                            localStorage.getItem('momoTotalAmount') || '0'
                        ),

                    movie: JSON.parse(
                        localStorage.getItem('momoMovie') || '{}'
                    ),

                    selectedCinema: JSON.parse(
                        localStorage.getItem('momoSelectedCinema') || '{}'
                    ),

                    selectedDate:
                        localStorage.getItem('momoSelectedDate') || '',

                    selectedShowtime: JSON.parse(
                        localStorage.getItem('momoSelectedShowtime') || '{}'
                    ),

                    selectedSeats: JSON.parse(
                        localStorage.getItem('momoSelectedSeats') || '[]'
                    ),

                    selectedFoods: JSON.parse(
                        localStorage.getItem('momoSelectedFoods') || '[]'
                    ),

                    foods: JSON.parse(
                        localStorage.getItem('momoFoods') || '[]'
                    ),
            ...
            console.error(
                '❌ Lỗi đọc momo data từ localStorage:',
                err
            );
        }

        // --------------------------------------------------------
        // Fallback: lastSuccessTicket
        // --------------------------------------------------------
    ...
    const hasVisitedMomoApp = useRef(
        localStorage.getItem(
            'momoHasVisited'
        ) === 'true'
    );
    ...
    const isReleasingSeatsRef =
        useRef(false);

    const otpInputsRef =
        useRef([]);

    const hasShownModalRef =
        useRef(false);
    ...
    const fetchTimeFromRedis =
        useCallback(async () => {
            ...
            try {
                const response =
                    await api.get(
                        `/api/momo/check-ttl/${tempBookingId}`
                    );
    ...
    const releaseSeatLocks =
        useCallback(async () => {
            if (
                isReleasingSeatsRef.current
            ) {
                return;
            }
            ...
            if (
                ...
                console.warn(
                    '⚠️ Socket không còn kết nối, không thể gửi release seat.'
                );
            ...
            try {
                ...
                console.log(
                    '🔓 [MOMO] Đã yêu cầu giải phóng ghế:',
                    {
                        showtimeId,
                        ownerToken
                    }
    ...
    const clearAllBookingData =
        useCallback(() => {
            const momoKeys = [
                ...
                'momoHoldExpiresAt',
                'momoSelectedSeats',
                'momoCurrentShowtimeId',
                'momoLastSuccessTicket',
                'momoTempBookingId',
                'momoSelectedFoods',
                'momoBookingTemp',
                'momoIsLocked',
    ...
    const cancelBookingOnServer =
        useCallback(async () => {
            ...
            try {
                ...
                console.log(
                    '✅ [MOMO] Temp booking cancelled'
                );
            } catch (err) {
                console.error(
                    '❌ [MOMO] Lỗi hủy temp booking:',
                    err
                );
            } finally {
                isCancellingRef.current =
    ...
    const handleExpireFlow =
        useCallback(async () => {
            ...
            console.log(
                '⏰ [MOMO] Payment/OTP expired'
            );

            // ----------------------------------------------------
            // 1. Release Redis seat lock
            // ----------------------------------------------------

            await releaseSeatLocks();

            // ----------------------------------------------------
            // 2. Cancel temp booking
            // ----------------------------------------------------
        ...
        const handleBeforeUnload =
            (e) => {
                if (
                    ...
                    e.returnValue =
                        'Bạn đang nhập OTP. Nếu rời trang, bạn sẽ mất tiến trình thanh toán!';
                }
            };
    ...
    useEffect(() => {
        ...
        if (
            currentSocketId &&
            currentSocketId !== ownerToken
        ) {
            console.warn(
                '⚠️ [MOMO] Owner token không khớp socket hiện tại:',
                {
                    ownerToken,
                    currentSocketId
                }
        ...
        async () => {
            if (isLocked) {
                openModal(
                    'error',
                    'TÀI KHOẢN BỊ KHÓA',
                    `Tài khoản đã bị khóa do nhập sai OTP quá 5 lần. Vui lòng thử lại sau ${formatTime(lockTimeLeft)}.`
                );
            ...
            try {
                ...
                if (
                    ...
                    openModal(
                        'error',
                        'QUÁ NHIỀU YÊU CẦU',
                        `Bạn đã gửi quá nhiều lần (tối đa 3 lần/5 phút). Vui lòng thử lại sau ${formatTime(remainingSeconds)}.`
                    );
                } else {
                    openModal(
                        'error',
    ...
    useEffect(() => {
        const initializeMomoApp =
            async () => {
                ...
                const hasOtpInStorage =
                    localStorage.getItem(
                        'momoOtpInput'
                    );

                const hasSentOtpFlag =
                    localStorage.getItem(
                        'momoHasSentOtp'
                    ) === 'true';
    ...
    const handleVerifyPayment =
        async () => {
            ...
            if (
                ...
                openModal(
                    'error',
                    'PHIÊN THANH TOÁN KHÔNG HỢP LỆ',
                    'Không tìm thấy phiên thanh toán. Vui lòng đặt vé lại.'
                );
            ...
            if (!ownerToken) {
                openModal(
                    'error',
                    'PHIÊN GIỮ GHẾ KHÔNG HỢP LỆ',
                    'Không tìm thấy phiên giữ ghế. Vui lòng quay lại chọn ghế.'
                );
            ...
            // ----------------------------------------------------
            // Socket phải còn đúng socket giữ ghế
            // ----------------------------------------------------
            ...
            if (
                ...
                openModal(
                    'error',
                    'MẤT KẾT NỐI GIỮ GHẾ',
                    'Phiên giữ ghế đã bị gián đoạn. Vui lòng quay lại đặt vé.'
                );

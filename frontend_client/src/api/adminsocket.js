...
// ✅ Y HỆT socket.js của user, chỉ đổi prefix
...
class AdminSocketService {
    ...
    disconnect(options = {}) {
        ...
        try {
            ...
            console.warn('⚠️ [ADMIN SOCKET] Disconnect error:', error);
        }
    ...
    getOwnerToken() {
        if (this.socket && this.socket.connected) return this.socket.id || null;
    ...
    emit(event, data = {}) {
        if (!this.socket || !this.socket.connected) {
            console.warn(`⚠️ [ADMIN SOCKET] Không thể emit "${event}"`);
            return false;
        }
